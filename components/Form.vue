  ...
  methods: {
    async onSubmit() {
      ...
      if(this.userInput.comment.trim().length > 0) {
        ...
        url = `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet&searchTerms=${ comment }&videoId=${ videoId }&key=${ process.env.API_KEY }&maxResults=100`;
      } else {
        // no comment
        url = `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet&videoId=${ videoId }&key=${ process.env.API_KEY }&maxResults=100`;
      }
